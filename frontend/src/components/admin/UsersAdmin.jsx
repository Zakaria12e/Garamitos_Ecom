    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-xl">
        ...
        <p className="text-xs text-gray-500 mb-1">
          {t('admin.users.deleteConfirm')} <span className="font-semibold text-black dark:text-white">{user.name}</span>?
        </p>
        <p className="text-xs text-gray-400 mb-5">{t('admin.users.deleteWarning')}</p>
...
export default function UsersAdmin() {
  ...
  const handleRoleToggle = async (u) => {
    ...
    try {
      const data = await usersApi.setRole(u._id, newRole)
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      await usersApi.delete(toDelete._id)
      setToDelete(null)
      load(search, page)
    } catch (err) {
      console.error(err)
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 bg-gray-100 dark:bg-gray-900 px-2 py-0.5 rounded-full">{total}</span>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-900 border border-transparent focus:border-gray-300 dark:focus:border-gray-700 rounded-md focus:outline-none w-56 transition-colors"
          />
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-gray-50 dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800">
              <tr>
                <th className="text-left px-4 py-2.5 font-medium text-gray-500">{t('admin.users.name')}</th>
                <th className="text-left px-4 py-2.5 font-medium text-gray-500">{t('admin.users.email')}</th>
                <th className="text-left px-4 py-2.5 font-medium text-gray-500">{t('admin.users.role')}</th>
                <th className="text-left px-4 py-2.5 font-medium text-gray-500">{t('admin.users.joined')}</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-900">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-12 text-gray-400">{t('admin.users.noUsers')}</td>
                </tr>
              ) : (
                users.map(u => (
                  <tr key={u._id} className="hover:bg-gray-50 dark:hover:bg-gray-950/50 transition-colors">
                    {/* Name */}
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-black dark:bg-white flex items-center justify-center shrink-0">
                          <span className="text-[9px] font-bold text-white dark:text-black">{u.name?.[0]?.toUpperCase()}</span>
                        </div>
                        <span className="font-medium truncate max-w-[140px]">{u.name}</span>
                        {u._id === me?._id && (
                          <span className="text-[9px] text-gray-400 border border-gray-200 dark:border-gray-700 px-1.5 rounded-full">you</span>
                        )}
                      </div>
                    </td>
                    {/* Email */}
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium transition-colors disabled:cursor-not-allowed ${
                          u.role === 'admin'
                            ? 'bg-black dark:bg-white text-white dark:text-black hover:bg-gray-700 dark:hover:bg-gray-200'
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                      >
                        <Trash2 size={13} />
                      </button>
                    </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-gray-400">{t('admin.orders.page', { current: page, total: totalPages })}</span>
